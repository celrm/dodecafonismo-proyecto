\section{Conteo de espectros seriales}\label{ch:espectros}
	\subsection{Espectros de las funciones $\{I,\ T,\ R\}$}
		...
		Para calcular el número de espectros seriales se redefinirán las funciones transformativas para una longitud serial arbitraria, $n$, que será mayor que 2. Para $n=0,\ 1$ y 2 se realizará el cálculo en el apartado \ref{s:trivcase}. Además, como las transposiciones siempre son distintas entre sí, siempre pertenecen al mismo espectro. Se tomarán a partir de ahora todas ellas como equivalentes, de manera que solo se necesita hacer el cálculo para  el conjunto de funciones $\{I,\ R\}$.
		
		Al calcular con permutaciones se trabajará siempre módulo $n$. La retrogradación sigue siendo $R(\sigma(m))=\sigma(-1-m)$. La inversión será $I(\sigma(m))=-\sigma(m)$, omitiendo la transposición habitual, ya que se toman las series transpuestas como equivalentes. De esta forma $-\sigma(m)+2\sigma(0)\equiv-\sigma(m)$. La retrogradación invertida es, por tanto, la composición de ambas: $RI(\sigma(m))={I}\circ{R}(\sigma(m))={I}\left({R}(\sigma(m))\right)=-\sigma(-1-m)$.			
...
\begin{center}
		\includegraphics{Serialismo-matematicas-diagrama-2.png}
\end{center}
	...
	\subsubsection[Elementos estables mediante $RI$]{Elementos estables mediante ${RI}$}
		...
		Si $n$ es par, $\sigma(m)\neq\sigma(-1-m)\ \forall m\in \mathbb{Z} / (n)$, ya que no hay elemento central. Sea ahora $\gamma=2k$ un número par. Como $2k\leq n$ y las permutaciones son suprayectivas, para algún $m$ se cumple que $\sigma(m)=k$. Se tiene entonces $k+\sigma(-1-m)=2k$, lo cual a su vez implica que $\sigma(-1-m)=k=\sigma(m)$. Como esto es una contradicción, $\gamma$ debe ser impar.
...
%			\ddiagram[no numbers, no arrow]{4,5,7,1,6,3,8,2,11,0,9,10}
			\includegraphics{Serialismo-matematicas-diagrama-3.png}
			\end{center}
		\end{figure}
		...
		En vez de expresar el sumatorio como ``para cada $\sigma$, el número de $\Psi$ que fijan $\sigma$'', se puede expresar como ``para cada $\Psi$, el número de $\sigma$ fijados por $\Psi$'' (que llamaremos ${Fij}(\Psi)$). La fórmula queda de esta manera:
...
%			\draw (90:1) -- (-45:1);
%			\end{tikzpicture}
			\includegraphics{Serialismo-matematicas-diagrama-4.png}
			\end{center}
%		\end{figure}
...
%				\end{tikzpicture}
%			\end{multicols}
		\includegraphics{Serialismo-matematicas-diagrama-5.png}
		\end{center}
...
%		\end{multicols}
\begin{center}
	\includegraphics{Serialismo-matematicas-diagrama-6.png}
\end{center}
...
%		\end{multicols}
\begin{center}
		\includegraphics{Serialismo-matematicas-diagrama-7.png}
\end{center}
...
%	\end{multicols}
\begin{center}
	\includegraphics{Serialismo-matematicas-diagrama-8.png}
\end{center}
...
%		\end{multicols}
\begin{center}
		\includegraphics{Serialismo-matematicas-diagrama-9.png}
\end{center}
...
%		\end{tikzpicture}
		\includegraphics{Serialismo-matematicas-diagrama-10.png}
		\end{center}
	...
	\label{s:trivcase}
		Con $n=0$ se da el caso de medefonismo. El grupo simétrico de orden 0 tiene $0!=1$ elemento. Por tanto, hay una sola posible serie, $\sigma$, que es la que no tiene ninguna nota. El medefonismo es comúnmente llamado silencio; se representará con una tabla vacía.
...
%		\end{array}\]
\begin{center}
			\includegraphics{Serialismo-matematicas-diagrama-11.png}
\end{center}
...
%		\end{array}\]
\begin{center}
					\includegraphics{Serialismo-matematicas-diagrama-12.png}
\end{center}
...
%		\sigma_1=\drow{1,0}\]
\begin{center}
					\includegraphics{Serialismo-matematicas-diagrama-13.png}
\end{center}
