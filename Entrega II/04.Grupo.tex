...
Como vimos en el anterior artículo~\cite{celia1}, las fórmulas de las transformaciones dodecafónicas habituales son:
		...
		Sin embargo, la importancia de estas definiciones radica en qué espectro serial forman, y no en cómo se nombra cada serie específica. No importa si la segunda serie que se usa en una obra se llama $I_6$ o $RI_{10}$, sino cuál es y qué relación tiene con la primera. La nomenclatura no es distinguible a un nivel auditivo, pero es, sin embargo, una útil herramienta para la teoría musical. Es por ello por lo que la forma en la que se nombran las series tiene relevancia y debería ser un reflejo de lo que significan. Aun así, sigue sin haber un convenio predominante y, de hecho, hay al menos dos que se usan a menudo.
		El primero, el método tradicional, se ha usado desde al menos 1945. El segundo, el método de tonos absolutos, fue concebido por George Perle~\cite{perle} en su libro \emph{Twelve-Tone Tonality} en 1977.
		
		En el método tradicional, $T_0$ se usa para la primera serie que se encuentra en la composición; es decir, es la serie original. En cambio, el método de tonos absolutos nombra las series $T$ basándose solamente en la nota en la que comienzan: $T_0$ se usa para la serie que comienza por un do, $T_1$ para el re, y así sucesivamente. En ambas, las series transpuestas $k$ semitonos de $T_0$ \textemdash sea cual sea esta\textemdash~se nombran como $T_{k}$. La composición de esta con otra función $\Psi$ se escribe como $\Psi_k$. 
		
		Estas nomenclaturas no caracterizan adecuadamente el objeto matemático que deben representar, es decir, funciones aplicadas a las series. Son nombres arbitrarios que además producen ambigüedad al añadir otras funciones \textemdash por ejemplo, si no conmutan\textemdash~o al intentar describirlo matemáticamente. Es por ello por lo que en este texto se usa la composición de funciones como convenio de notación.
		...
		Esta nueva transformación, cuya aparición resulta natural tras las otras tres, se llama \textit{desplazamiento cíclico}. Inventada y usada por Alban Berg~\cite{cyclic} \textemdash de quien se hablará en la \hyperref[ch:serialismo]{sección 3}\textemdash~y en algunas de las primeras obras de Schoenberg, $C^{k}$ desplaza el comienzo de la serie $k$ posiciones más allá:
	...
	\subsection{Diagramas de reloj}
		...
		\begin{figure}[h]
			\begin{center}			
				...
				\includegraphics[scale=1.28]{other/Serialismo-matematicas-diagrama-2-1.png}
%			\includegraphics[scale=0.5]{Serialismo-matematicas-diagrama-2-1.png}
	...
	\begin{center}
		\includegraphics{other/Serialismo-matematicas-diagrama-2-2.png}
...
%				\newpage
		\begin{center}
			\includegraphics{other/Serialismo-matematicas-diagrama-2-3.png}
...
%			\end{multicols}
		...
		\begin{center}
		\includegraphics{other/Serialismo-matematicas-diagrama-2-4.png}
...
\begin{center}
\includegraphics{other/Figura-ddmark.png}
%\includegraphics[scale=0.45]{Figura-ddmark.png}
% \verb|\ddiagram[up=4, name=$T^0$]{4,5,7,1,6,3,8,2,11,0,9,10}|
\end{center}
\noindent y en este caso \texttt{up=4} no es necesario, ya que por defecto se coloca arriba la primera nota de la serie. El comando se encuentra en el paquete de \LaTeX{} \texttt{ddphonism}, disponible en el enlace \url{https://www.ctan.org/pkg/ddphonism}.
...
%	\subsection[El grupo: $D_{12}\times D_{12}$]{El grupo: $\bm{D_{12}\times D_{12}}$}
	\subsection{El grupo: $D_{12}\times D_{12}$}
		...
		\begin{center}
		\includegraphics{other/Serialismo-matematicas-diagrama-2-5.png}
...
%		\end{tikzpicture}
		...
		Las dos primeras figuras describen esto mismo: la representación de la serie original y la representación de la permutación mediante las flechas, que se mantendrán constantes en el resto de figuras.
			...
			\begin{center}
				\includegraphics{other/Serialismo-matematicas-diagrama-2-6.png}
...
%		\tikzstyle rotateArrow=[decoration={markings,mark=at position 0.9999 with {\arrow[scale=1,>=triangle 45]{>}}},postaction={decorate}]
		...
		\begin{center}
				\includegraphics{other/Serialismo-matematicas-diagrama-2-7a.png}
				
				\includegraphics{other/Serialismo-matematicas-diagrama-2-7b.png}
...
%		\end{multicols}
	...
	He creado un comando en \LaTeX{} que dibuja estos diagramas diédricos dada su serie original y las funciones aplicadas a ella: \texttt{t}, \texttt{s}, \texttt{c} y \texttt{v}. Se aplican en ese mismo orden, y por defecto están a 0. 
	\begin{center}
		\includegraphics{other/Figura-ddmark2.png}
		%\includegraphics[scale=0.45]{Figura-ddmark.png}
		% \verb|\ddiagram[up=4, name=$T^0$]{4,5,7,1,6,3,8,2,11,0,9,10}|
	\end{center}
	\noindent
	El comando se encuentra en el paquete de \LaTeX{} \texttt{ddphonism}, disponible en el enlace \url{https://www.ctan.org/pkg/ddphonism}.
...
\begin{center}
	\includegraphics{other/Serialismo-matematicas-2-8a.png}
	
	\includegraphics{other/Serialismo-matematicas-2-8b.png}
	
	\includegraphics{other/Serialismo-matematicas-2-8c.png}
	
	\includegraphics{other/Serialismo-matematicas-2-9a.png}
	
	\includegraphics{other/Serialismo-matematicas-2-9b.png}
	
	\includegraphics{other/Serialismo-matematicas-2-9c.png}
...
%		\includegraphics[scale=0.6]{Serialismo-matematicas-diagrama-2-10.png}
		\includegraphics{other/Serialismo-matematicas-2-10.png}
\end{center}
...
%		\end{multicols}
		...
		\begin{center}
		\includegraphics{other/Serialismo-matematicas-2-12.png}
...
\begin{center}
		\includegraphics{other/Serialismo-matematicas-2-11.png}
\end{center}
...
%		\end{multicols}
		...
		\begin{center}
		\includegraphics{other/Serialismo-matematicas-2-14.png}
...
%		\end{align*}
		...
		\begin{center}
		\includegraphics{other/Serialismo-matematicas-2-13.png}
