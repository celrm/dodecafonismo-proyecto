...
\it
\begin{enumerate}
\item La serie \it{(sobre la que se construye la obra dodecafónica)} consta de las doce notas de la escala cromática dispuestas en un orden lineal específico. 
...
\rm
		...
		Los dos primeros postulados expresan que una obra dodecafónica fundamenta su estructura sobre una {permutación} de la escala de doce semitonos. Dicha permutación $\sigma$ es una biyección\xxx{Sugerencia: ¿Ponemos la definición de biyección en la sección de términos matemáticos?} del conjunto numerado de las doce notas \{Do = 0, Do\# = 1, Re = 2, Re\# = 3, Mi = 4, Fa = 5, F\# = 6, Sol = 7, Sol\# = 8, La = 9, La\# = 10, Si = 11\} consigo mismo, y se representa de esta forma:
...
Los otros dos postulados restante son:
...
\rm	
	...
	\subsubsection{Transposiciones}
		...
		Históricamente, la notación $\Psi_{k}$, $\Psi^{k}$ o $\Psi({k})$ se ha usado en sustitución de la composición de la transposición $T^{k}$ y otra función $\Psi$, en el respectivo orden: $\Psi^{k}=\Psi \circ T^{k} = \Psi(T^{k})$. Sin embargo, esta notación es especialmente ambigua y confusa, sobre todo al trabajar con funciones no conmutativas \textemdash cuando importa el orden en el que estén $T$ y $\Psi$\textemdash. Por ello, es preferible ceñirse a la notación estrictamente matemática; es decir, a la composición de funciones, aun omitiendo el símbolo $\circ$: \cancel{$\Psi_{k}$}, \cancel{$\Psi^{k}$}, \cancel{$\Psi({k})$} $\rightarrow \Psi T^{k}$.\xxx{Estos últimos símbolos quedan confusos. No se sabe bien qué significa la cancelación. Sugiero reescribir esta parte.}
...
%===================
	\subsubsection{Retrogradación}
		...
		La \emph{retrogradación}\xxx{Sugerencia de formato: yo pondría las definiciones en negrita; las itálicas se usarán para la ironía, los títulos de obras, y otras.} consiste en leer la serie original desde la nota final hacia atrás, es decir, aplicar a la serie una simetría especular. De este modo, la primera nota irá al último puesto, la segunda al penúltimo, y así sucesivamente. La serie retrógrada se construye según la siguiente fórmula:
	...
	\subsubsection{Inversión}
		...
		\begin{align*}
		I(\sigma(0))=-\sigma(0)+\lambda&=\sigma(0)\\
